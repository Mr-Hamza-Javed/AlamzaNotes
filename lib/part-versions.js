...
AlamzaParts.register(class {
  ...
  createVersion(msg) {
    const p = this.page();
    if (!p) return;
    if (!this.canWriteHistory(p)) {
      this.toast(p.trashed ? 'Restore this page from the trash before snapshotting it'
                           : 'You do not have permission to snapshot this page');
      return;
    }
    ...
    this._verLock = true;
    const pid = p.id;
    /* A deep capture can take several fetches. Without a state for it the Save
       button simply stopped responding — the lock silently swallowed the
       click — so the dialog now says it is working. */
    this.setState({ capturing: true });
    const done = () => { this._verLock = false; this.setState({ capturing: false }); };
  ...
  /* Describe what actually moved. The root summary alone would read "No
     ...
     ONE builder, because the dialog previews this text under "If you leave it
     blank:" and the two had drifted: the preview ended "· 2 nested" where the
     saved message ended "· changes in 2 nested pages". A preview that is not
     the thing being saved is worse than no preview. */
  autoMessage(prevRoot, blocks, touched, gone) {
    const text = ADiff.summary(prevRoot || [], blocks || []);
    const parts = [];
    ...
    /* `summary` has already run the diff; asking `stats` the same question
       again would run it a second time, and this is built on every render
       while the dialog is open */
    if (text === 'No content changes') {
      const s0 = parts.join(' · ');
  ...
  /* What a capture on this page would cover, for the dialog to say so before
     the click. The copy used to read "This snapshots the page as it is right
     now" over a capture that also takes every sub-page and every table. */
  captureScope(pageId) {
    const ids = this.snapshotIds(pageId);
    let dbs = [];
    ids.forEach(id => {
      const pg = this.state.pages[id];
      if (pg && pg.blocks) this.dbIdsIn(pg.blocks).forEach(d => { if (dbs.indexOf(d) < 0) dbs.push(d); });
    });
    return { pages: ids.length, nested: ids.length - 1, dbs: dbs.length };
  }
  ...
  authorVersion(pageId, msg) {
    ...
    const text = auto ? this.autoMessage(prevRoot, p.blocks, touched, gone) : msg.trim();
    const vid = uid('v');
  ...
  restore(vid, mode) {
    ...
    if (!this.vsnap(vid)) {
      if (mode === 'ro') this.setState({ roVersion: vid, modal: null, sheet: null });
    ...
    /* the versions panel stays open: it is how the reader picks the next one,
       and closing it meant reopening it between every two versions. The mobile
       sheet does close — it covers the page being previewed. */
    if (mode === 'ro') { this.setState({ roVersion: vid, modal: null, sheet: null }); return; }
    /* Reading a version is not writing one — `isReadOnly()` is true throughout
       a preview and "Restore…" is offered from inside it, so the question is
       the role and the Trash, asked here rather than at the three call sites */
    if (!this.canWriteHistory(p)) {
      this.setState({ modal: null });
      this.toast(p.trashed ? 'Restore this page from the trash before restoring a version'
                           : 'You do not have permission to change this page');
      return;
    }
  ...
  /* Choosing a side of the diff has to FETCH that side. openDiff prefetched
     the two it opened with and nothing else did: the dropdown handlers only
     set state, and the pending branch of diffScope fetches page BODIES, not
     payloads — so picking another version left the pane on "comparing…"
     for as long as the modal stayed open. */
  pickDiffSide(which, id) {
    this.setState(which === 'a' ? { diffA: id } : { diffB: id });
    if (id && id !== 'current') this.ensureVersion(id);
  }
  ...
  diffStyle(b) {
    const t = b ? b.type : 'p';
    /* ADiff flattens the tree, so a row can come from inside a toggle or a
       column. Its own indent plus that depth is how it reads as nested rather
       than as a sibling of everything else. */
    const depth = (b && b.__depth) || 0;
    const ind = (b && b.indent) || 0;
    return {
