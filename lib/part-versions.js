...
AlamzaParts.register(class {
  ...
  createVersion(msg) {
    ...
    /* The HISTORY is the first of those things, and the most dangerous to get
       wrong. Version metadata lives in its own node, so a page whose list has
       not been fetched presents as `versions: []` — and authoring against that
       gives the new snapshot the number 1 and a list of one, which push() then
       writes over every real snapshot on the server. So the list is read
       before anything is authored. One retry, then say so: an unbounded
       re-run would sit in a fetch loop billing a read per pass. */
    if (!this.versionMetaKnown(p.id)) {
      this.ensureVersionMeta(p.id).then(() => {
        done();
        if (this.versionMetaKnown(p.id)) this.createVersion(msg);
        else this.toast('Could not load this page\u2019s history \u2014 try again in a moment');
      }, done);
      return;
    }
