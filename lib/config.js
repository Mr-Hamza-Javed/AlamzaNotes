...
window.ALAMZA_CONFIG = {
  ...
  backends: {
     ...
     *   timeoutMs how long one request may take before it is abandoned
     *             (default 20000). Without a deadline a dropped connection
     *             leaves the app looking frozen for minutes.
   ...
   *   strictContract A DEVELOPMENT AID, off by default. On startup it writes a
   *                 handful of probe documents, reads them back and deletes
   *                 them, to check the selected backend really answers every
   *                 call the app makes — then says so in the console if it
   *                 does not. Turn it on while you are WRITING a backend and
   *                 turn it off again: it writes to the signed-in account's own
   *                 workspace (under `probe/`, which the app never uses), and
   *                 there is no reason to do that on every load forever.
   */
  tuning: {
    cacheBudget: 2400000,
    pushDelayMs: 2200,
