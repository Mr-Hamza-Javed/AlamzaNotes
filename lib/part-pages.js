...
AlamzaParts.register(class {
  ...
  openPage(id, anchor) {
    ...
    this.histFlush();
    /* a shared page whose mirror is still on its fuse goes out now, rather
       than leaving its readers on the previous version */
    if (this.flushShareSync) this.flushShareSync();
  ...
  vsnap(vid) {
    ...
    if (this._vb && this._vb[vid]) { this.touchVersion(vid); return this._vb[vid]; }
    const v = this.versionById(vid);
    return v && v.blocks ? this.normSnap(v.blocks) : null;
  }
  /* ---- the payload memo is bounded ------------------------------------
     A snapshot payload is the heaviest object in the workspace — a whole
     subtree plus every table it embeds — and this map held every one the
     session ever touched, for as long as the tab stayed open, across sign-out
     and across a switch into the demo workspace. Browsing a long history was
     enough to keep tens of megabytes alive with nothing referring to it.
     Least-recently-used goes first; the payloads are all re-fetchable. */
  touchVersion(vid) {
    const lru = this._vbLru = this._vbLru || [];
    const i = lru.indexOf(vid);
  ...
  trimVersionCache() {
    this._vb = this._vb || {};
    const lru = this._vbLru = this._vbLru || [];
    Object.keys(this._vb).forEach(id => { if (lru.indexOf(id) < 0) lru.push(id); });
    while (lru.length > VERSION_CACHE) {
      const drop = lru.shift();
      /* never evict what the reader is looking at right now */
      if (drop === this.state.roVersion) { lru.push(drop); continue; }
      delete this._vb[drop];
    }
  }
  /* Signing out, or stepping into the demo workspace, must not leave the
     previous account's snapshots in memory. Everything here is a memo of
     something stored elsewhere, so dropping it costs a re-fetch and nothing
     more. */
  forgetVersions() {
    this._vb = {}; this._vbP = {}; this._vbGone = {}; this._vbLru = [];
    this._vmP = {}; this._verGone = {};
    this._chKey = null; this._chIds = {};
  }
  vblocks(vid, pageId) {
  ...
  /* A payload is fetched at most once per session, hit OR miss.
     A miss was not remembered, and the two places that ask are a render value
     (`extraVals`, `changedIds`) and every remote delta (`primeLatestVersion`),
     so a payload the server does not hold — a migration gap, a write that
     never landed — was re-read on every keystroke pause and every edit made on
     another device. Downloads are the billed number in this app; that is the
     one thing lib/store.js exists to keep small. */
  ensureVersion(vid) {
    if (!vid || vid === 'current') return Promise.resolve(null);
    const have = this.vsnap(vid);
    if (have) return Promise.resolve(have);
    this._vbGone = this._vbGone || {};
    if (this._vbGone[vid]) return Promise.resolve(null);
    const p = this.page();
    ...
    this._vbP[vid] = AStore.getVersionBlocks(p.id, vid).then((raw) => {
      ...
      if (snap) { this._vb[vid] = snap; this.trimVersionCache(); this.forceUpdate(); }
      /* remembered as absent, not as "ask again in a moment" */
      else this._vbGone[vid] = 1;
      return snap;
    }, () => { delete this._vbP[vid]; this._vbGone[vid] = 1; return null; });
  ...
  /* A preview whose version has stopped existing — deleted on another device,
     or a history that arrived without it — used to sit there silently: with no
     version to resolve, activeBlocks() fell through to the LIVE page while the
     banner still read "Read-only preview of an old version" and isReadOnly()
     still refused every edit. Live content, an old label, and no way to type. */
  checkRoVersion() {
    const ro = this.state.roVersion;
    if (!ro || this.versionById(ro)) return;
    this.setState({ roVersion: null });
    this.toast('That version is no longer in this page\u2019s history');
  }
  syncVersionMeta() {
    ...
    this.ensureVersionMeta(id).then(() => { this.checkRoVersion(); this.primeLatestVersion(); });
  ...
  deleteVersion(vid, pageId) {
    ...
    const p = this.state.pages[id];
    if (!p) return;
    if (!this.canWriteHistory(p)) { this.toast('You do not have permission to change this page'); return; }
  ...
  cacheVersion(pageId, vid, snap) {
    this._vb = this._vb || {};
    this._vb[vid] = snap;
    /* a payload that has arrived is no longer a miss, whichever route it came
       by — an author, a restore, a duplicate */
    if (this._vbGone) delete this._vbGone[vid];
    if (AStore.putVersionBlocks) AStore.putVersionBlocks(pageId, vid, snap);
    this.trimVersionCache();
  }
  ...
  versionMetaKnown(pageId) {
    ...
    if (!AStore.cloud || !AStore.loadVersionMeta) return true;
