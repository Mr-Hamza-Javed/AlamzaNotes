  ...
  extraVals() {
    ...
    if (mu.kind === 'block') {
      ...
      menuItems = [
        ...
        rule(),
        /* a row page IS a row — say so, so nobody expects the table to keep it */
    ...
    return {
      ...
      /* A password changes what gets STORED, not just what the reader is
         asked — the published copy is sealed with it — so both of these
         re-publish rather than only editing the page. */
      setPw: (e) => { patchShare({ password: e.target.value }); },
      commitPw: () => { if (sh.published) this.republish(s.pageId); },
      togglePw: () => {
      ...
      /* pending invitations addressed to the signed-in account — the ones this
         account SENT are somebody else's mail, and showing them here let an
         owner accept their own invite and demote themselves */
      inbox: (s.invites || []).filter(i => i.status === 'pending' && this.invitedMe(i)).map(i => ({
      ...
      hasInbox: (s.invites || []).some(i => i.status === 'pending' && this.invitedMe(i)),
      inboxCount: (s.invites || []).filter(i => i.status === 'pending' && this.invitedMe(i)).length,
