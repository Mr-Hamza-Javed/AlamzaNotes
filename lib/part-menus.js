  ...
  extraVals() {
    ...
    if (mu.kind === 'block') {
      ...
      menuItems = [head('Turn into')].concat(
        CATALOG.filter(c => ['p', 'h1', 'h2', 'h3', 'ul', 'ol', 'todo', 'toggle', 'quote', 'callout', 'code', 'math'].indexOf(c.type) >= 0)
    ...
    const emojiList = m.kind !== 'emoji' ? [] : EMOJI.filter(e => !eq || e.name.includes(eq)).map(e => ({
      ...
      pick: () => {
        ...
        else this.patchPage(s.pageId, { icon: e.ch }, true);
