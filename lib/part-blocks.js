...
AlamzaParts.register(class {
  ...
  turnInto(id, type) {
    this.mutate(bs => {
      ...
      f.block.type = type;
      if (type === 'code' && !f.block.lang) f.block.lang = 'plaintext';
      if (type === 'callout' && !f.block.icon) f.block.icon = '💡';
      if (type === 'toggle' && !f.block.children) f.block.children = [{ id: uid('b'), type: 'p', text: '', indent: 0 }];
    });
  ...
  insertOfType(afterId, type) {
    ...
    if (f && f.block.type === 'p' && !(f.block.text || '').trim()) {
      this.mutate(bs => {
        ...
        g.block.type = type; g.block.text = '';
