      ...
      for (;;) {
        ...
        ids.forEach(function (id) {
          ...
          var e = toIdx({
            ...
            publicId: p.publicId, locked: p.locked, blocks: blocks
          });
          e.w = WID;
      ...
      for (;;) {
        ...
        for (var i = 0; i < dids.length; i++) {
          ...
          var m = toDbMeta({ name: d.name, icon: d.icon, props: d.props, views: d.views,
                             defaultView: d.defaultView, pageHidden: d.pageHidden, rows: rows });
          m.w = WID;
  ...
  var AStore = {
    ...
    push: function () {
      ...
      Object.keys(st.pages || {}).forEach(function (id) {
        var p = st.pages[id];
        if (!p.blocks) return;                    // never opened: cannot be dirty
      ...
      this.stats.writes++;
      try { fb.update(at(uid), patch); notifySave('saved'); }
      catch (e) { console.warn('[AStore] write failed', e); notifySave('error'); }
    },
    ...
    /* the app calls this when it authors a snapshot, so a page whose history
       we created this session is authoritative without a read */
    markVersionMeta: function (pageId) { vmetaSeen[pageId] = true; },
    ...
    async signOut() {
      ...
      vcache = {}; bodies = {}; rowsMem = {}; digests = null; remote = null;
      vmetaSeen = {};
      this._layoutOk = null;
