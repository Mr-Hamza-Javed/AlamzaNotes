...
(function () {
  ...
  async function openBackend() {
    ...
    /* An adapter that answers the six calls but behaves differently from the
       others is the one failure this design cannot survive, so it is checked
       rather than assumed — loudly, in the console, and only when asked for.
       It writes and deletes under a probe uid, never a real workspace. */
    if (conf.tuning.strictContract) {
      D.conformance(backend).then(function (bad) {
        if (bad.length) console.warn('[AStore] the "' + opened.name +
  ...
  /* watch one collection of the workspace, metering everything that arrives */
  function child(uid, col, fn, kind, which) {
    var h = {};
    if (which === 'removed') h.removed = function (k) { fn(k, null); };
    else h[which === 'changed' ? 'changed' : 'added'] = function (k, v) { fn(k, meter(v, kind)); };
  ...
  function hasAnyChild(uid, sub) {
    return backend.readPage(WS(uid), sub, null, 1)
      .then(function (v) { return !!(v && Object.keys(v).length); });
  }
  ...
  async function migrate(uid, onProgress) {
    /* The completion test must NOT be "the old nodes are gone". Cleanup is
       ...
       silently erase it. */
    var stamp = await backend.read(WS(uid), 'layout');
    ...
    var meta = await backend.read(WS(uid), 'meta');
    if (!meta) {
      var prefs = await backend.read(WS(uid), 'prefs');
      var ws = await backend.read(WS(uid), 'workspace');
      var inv = await backend.read(WS(uid), 'invites');
  ...
  function subscribe(uid) {
    ...
    child(uid, 'idx', function (id, e) { markIndexReady(); considerIdx(id, e); }, 'idx', 'added');
    child(uid, 'idx', considerIdx, 'idx', 'changed');
    child(uid, 'idx', function (id) {
    ...
    child(uid, 'dbmeta', considerMeta, 'dbmeta', 'added');
    child(uid, 'dbmeta', considerMeta, 'dbmeta', 'changed');
    child(uid, 'dbmeta', function (id) {
    ...
    child(uid, 'dbrev', considerRev, 'dbrev', 'added');
    child(uid, 'dbrev', considerRev, 'dbrev', 'changed');
  ...
  /* ------------------------------------------------------------------ api */
  var AStore = {
    /* KEPT FOR COMPATIBILITY, and it no longer means Firebase. Eighteen places
       in the app ask `AStore.mode === 'firebase'` and every one of them means
       "is there a server?". Renaming it would be a rename in eighteen files for
       no behaviour; `cloud` is the honest name and both say the same thing. */
    mode: useCloud ? 'firebase' : 'local',
    cloud: useCloud,
    ...
    enterDemo: function () {
      ...
      this.demo = true; this.mode = 'local'; this.cloud = false; useCloud = false;
    ...
    async _doInit() {
      ...
      try {
        ...
        this.mode = 'local'; this.cloud = false; useCloud = false;
        this.user = local.user();
        this.ready = true;
    ...
    publishPage: function (slug, payload, password) {
      if (this.mode !== 'firebase' || !backend || !this.user || !slug) return Promise.resolve(false);
    ...
    sendInviteTo: function (email, invite) {
      ...
      if (this.mode !== 'firebase' || !backend || !this.user || !key) return Promise.resolve(false);
    ...
    loadInbox: function () {
      ...
      if (this.mode !== 'firebase' || !backend || !this.user || !key) return Promise.resolve([]);
      var self = this;
    ...
    dropInvite: function (inviteId, email) {
      ...
      if (this.mode !== 'firebase' || !backend || !this.user || !key || !inviteId) return Promise.resolve(false);
    ...
    putShared: function (pageId, payload, memberEmails) {
      if (this.mode !== 'firebase' || !backend || !this.user || !pageId) return Promise.resolve(false);
      var self = this;
      var m = {};
    ...
    loadShared: function (pageId) {
      if (this.mode !== 'firebase' || !pageId) return Promise.resolve(null);
    ...
    loadDigests: function () {
      ...
      if (this.mode !== 'firebase' || !backend || !this.user) return Promise.resolve(null);
      try {
    ...
    dropVersionBlocks: function (pageId, vId) {
      ...
      var i = vcacheLru.indexOf(k);
      if (i >= 0) vcacheLru.splice(i, 1);
      if (this.mode === 'firebase') vqueue[k] = null;
    },

    retryMigration: function () {
      if (this.mode !== 'firebase' || !backend || !this.user) return Promise.resolve('error');
    ...
    purgeDatabases: function (ids) {
      if (this.mode !== 'firebase' || !backend || !this.user) return Promise.resolve(0);
    ...
    async signInWithGoogle() {
      /* The name is kept because the sign-in button calls it; WHICH provider
         answers is lib/config.js's business, not this method's. */
      if (this.mode === 'firebase' && auth) {
        try {
          var signed = await auth.signIn();
    ...
    async signOut() {
      ...
      vmetaSeen = {};
      this._layoutOk = null;
      if (this.mode === 'firebase' && auth) { await auth.signOut(); return; }
