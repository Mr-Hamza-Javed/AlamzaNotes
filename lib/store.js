...
(function () {
  ...
  var AStore = {
    ...
    sendInviteTo: function (email, invite) {
      ...
      if (this.mode !== 'firebase' || !fb || !this.user || !key) return Promise.resolve(false);
      this.stats.up += JSON.stringify(invite).length;
      this.stats.writes++;
      return fb.set(fb.ref(fb.db, 'inbox/' + key + '/' + invite.id), invite)
    ...
    loadInbox: function () {
      ...
      if (this.mode !== 'firebase' || !fb || !this.user || !key) return Promise.resolve([]);
      var self = this;
      return fb.get(fb.ref(fb.db, 'inbox/' + key)).then(function (snap) {
        var v = (snap && snap.val()) || {};
    ...
    dropInvite: function (inviteId, email) {
      ...
      if (this.mode !== 'firebase' || !fb || !this.user || !key || !inviteId) return Promise.resolve(false);
      return fb.set(fb.ref(fb.db, 'inbox/' + key + '/' + inviteId), null)
    ...
    putShared: function (pageId, payload, memberEmails) {
      if (this.mode !== 'firebase' || !fb || !this.user || !pageId) return Promise.resolve(false);
      var self = this;
      var m = {};
      ...
      this.stats.writes++;
      return fb.set(fb.ref(fb.db, 'shared/' + pageId), body)
        .then(function () { return true; })
    ...
    loadShared: function (pageId) {
      ...
      var self = this;
      return fb.get(fb.ref(fb.db, 'shared/' + pageId)).then(function (snap) {
        var v = snap && snap.val();
        if (!v) return null;
        self.stats.down += JSON.stringify(v).length;
        self.stats.reads++;
    ...
    dropShared: function (pageId) {
      if (this.mode !== 'firebase' || !fb || !this.user || !pageId) return Promise.resolve(false);
      return fb.set(fb.ref(fb.db, 'shared/' + pageId), null)
        .then(function () { return true; }).catch(function () { return false; });
    },
