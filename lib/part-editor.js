...
AlamzaParts.register(class {
  ...
  onKey(id, e) {
    ...
    if ((e.key === 'ArrowLeft' || e.key === 'ArrowRight') && !mod && !e.shiftKey && off != null) {
      ...
      /* "At the end" has to mean what the READER sees. Typing `**bold**`
         leaves the caret between the two closing asterisks — inside a
         display:none span, with no rect of its own — and from there the
         browser cannot move it at all: → did nothing, for ever, from a
         position reached by typing one bold word. Everything between the
         caret and the end being a hidden delimiter IS the end, the same way
         ⌫ reads the other edge of the block. (Mid-text needs no help:
         Chromium steps over a whole marker run in one press.) Code and
         equations hold literal text, so no marker map applies to them. */
      const hidden = multi ? null : AMD.markMap(text);
      const allHidden = (from, to) => {
        if (!hidden) return false;
        for (let k = from; k < to; k++) if (!hidden[k]) return false;
        return true;
      };
      const atEdge2 = back ? (off === 0 || allHidden(0, off))
                           : (off === text.length || allHidden(off, text.length));
      if ((!sel3 || sel3.isCollapsed) && atEdge2) {
    ...
    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
      if (e.shiftKey) { this._col = null; return; }
