  ...
  onKey(id, e) {
    ...
    if (e.key === 'Enter' && !e.shiftKey && b.type !== 'code' && b.type !== 'math') {
      ...
      if (b.type === 'toggle' && !text.trim()
        ...
        this.mutate(bs => { const g = this.locate(id, bs); if (g) { g.block.type = 'p'; g.block.indent = 0; delete g.block.children; delete g.block.collapsed; } });
        return;
      }
    ...
    if (e.key === 'Backspace' && off === 0 && !e.shiftKey) {
      const sel = window.getSelection();
      if (sel && !sel.isCollapsed) return;
      if (b.type !== 'p') { e.preventDefault(); this.mutate(bs => { const g = this.locate(id, bs); if (g) { g.block.type = 'p'; g.block.indent = 0; g.block.checked = false; } }); return; }
