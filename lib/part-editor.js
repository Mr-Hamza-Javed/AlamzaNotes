...
AlamzaParts.register(class {
  ...
  syncDom() {
    ...
    if (this._publicEl && this._publicEl.isConnected) {
      /* A real visitor has no workspace — they arrived at a link and the whole
         page came down in one read, so the reader draws `pubDoc` when there is
         one and falls back to the local page for the owner's own preview. */
      const pub = this.state.pubDoc;
      const p2 = pub ? null : this.page();
      const blocks = pub ? (pub.b || []) : (p2 ? p2.blocks : []);
      const key = pub
        ? 'pub:' + (this.state.pubSlug || '') + ':' + (pub.u || 0) + ':' + (pub.b || []).length
        : this.state.pageId + ':' + (p2 ? (p2.blocks || []).length : 0) + ':' + (p2 ? p2.updatedAt : 0);
      if (this._publicEl.__k !== key) {
        this._publicEl.__k = key;
        this._publicEl.innerHTML = this.readHtml(blocks, pub ? { pages: {}, dbs: pub.d || {} } : undefined);
      }
    }
