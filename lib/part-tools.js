...
AlamzaParts.register(class {
  ...
  histMark() { if (!this._histT) this.syncTail(); }
  histLater() {
  ...
  applyHist(i) {
    ...
    const from = h.idx >= 0 && h.stack[h.idx] ? JSON.parse(h.stack[h.idx]) : null;
    h.idx = i;
    ...
    this.setState(s => {
      ...
      return { pages, dbs: this.dbDelta(s.dbs, from && from.d, snap.d), blockSel: [] };
    }, () => this.persist());
  }
  /* Both keys begin by filing whatever is still pending. Typing files its
     history on a 900ms debounce, so a character struck a moment ago is not in
