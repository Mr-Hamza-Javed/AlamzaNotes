...
var HIST_STEPS = 90, HIST_PAGES = 12, HIST_BYTES = 24e6;
...
AlamzaParts.register(class {
  ...
  histJson() {
    const p = this.page();
    if (!p) return null;
    return { b: JSON.stringify({ b: p.blocks, t: p.title }), d: JSON.stringify(this.dbsUsedBy(p.blocks) || null) };
  }
