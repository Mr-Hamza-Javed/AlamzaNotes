...
AlamzaParts.register(class {
  ...
  escapeKey() {
    ...
    if (s.modal) { this.setState({ modal: null }); return true; }
  ...
  /* the published copy is a snapshot, so a change here has to be sent */
  republish(pageId) {
    const p = this.state.pages[pageId];
    const sh = p && p.share;
    if (!p || !sh || !sh.published || AStore.mode !== 'firebase') return;
    this.publishSnapshot(p).then(payload =>
      AStore.publishPage(sh.slug || pageId, payload, sh.password || null)
        .then(okPub => this.toast(okPub ? 'Published copy updated' : 'Could not update the published copy')));
  }
  ...
  sendInvite(pageId, email, role) {
    ...
    const inv = {
      ...
      /* who it is FOR. Without this the outbox and the inbox were one list:
         an invite you sent showed up in your own inbox, and accepting it made
         myRole() hand you that role on your own page — a viewer of your own
         note, unable to edit it. */
      to: String(email).toLowerCase(),
      fromEmail: ((this.state.user && this.state.user.email) || '').toLowerCase(),
  ...
  /* is this invite addressed to the person using the app right now? */
  invitedMe(inv) {
    const me = ((this.state.user && this.state.user.email) || '').toLowerCase();
    if (!inv) return false;
    /* an invite written before `to` existed cannot be attributed, and the safe
       reading of "unknown recipient" is "not me" — the alternative locked
       owners out of their own pages */
