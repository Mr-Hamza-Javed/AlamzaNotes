...
AlamzaParts.register(class {
  ...
  republish(pageId) {
    const p = this.state.pages[pageId];
    const sh = p && p.share;
    if (!p || !sh || !sh.published || !AStore.cloud) return Promise.resolve(false);
  ...
  syncShared(pageId, members, drop) {
    ...
    if (!AStore.cloud) return Promise.resolve(false);
    if (drop || !list.length) {
  ...
  openShared(pageId, andOpen) {
    if (!AStore.loadShared || !AStore.cloud) return Promise.resolve(null);
