...
AlamzaParts.register(class {
  ...
  readHtml(blocks) {
    ...
    (blocks || []).forEach(b => {
      ...
      if (t === 'h1') out.push('<h2 style="font-size:28px;font-weight:700;letter-spacing:-.018em;margin:34px 0 8px;line-height:1.25">' + txt + '</h2>');
        ...
        out.push('<details style="margin:10px 0"><summary style="cursor:pointer;font-weight:500">' + txt + '</summary><div style="padding:8px 0 0 20px">' + this.readHtml(b.children || []) + '</div></details>');
      } else if (t === 'table') {
        const rows = b.rows || [];
        if (rows.length) {
  ...
  histFor() {
    const p = this.page();
    if (!this._hist || this._hist.pid !== (p && p.id)) this._hist = { pid: p && p.id, stack: [], idx: -1 };
    return this._hist;
  }
  ...
  histJson() {
    const p = this.page();
    if (!p) return null;
    return JSON.stringify({ b: p.blocks, t: p.title, d: this.dbsUsedBy(p.blocks) || null });
  }
  syncTail() {
    const h = this.histFor(), j = this.histJson(); if (!j) return;
    if (h.idx >= 0 && h.stack[h.idx] === j) return;
    h.stack = h.stack.slice(0, h.idx + 1);
    h.stack.push(j);
    if (h.stack.length > 90) h.stack.shift();
    h.idx = h.stack.length - 1;
  }
  applyHist(i) {
    const h = this.histFor(), p = this.page();
    if (!p || i < 0 || i >= h.stack.length) return;
    h.idx = i;
    const snap = JSON.parse(h.stack[i]);
    this._els = {}; this._refs = {}; this._mathEls = {}; this._mathRefs = {};
    this.setState(s => {
      const pages = { ...s.pages };
      pages[p.id] = Object.assign({}, pages[p.id], { blocks: snap.b, title: snap.t, updatedAt: Date.now() });
      const dbs = snap.d ? Object.assign({}, s.dbs, JSON.parse(JSON.stringify(snap.d))) : s.dbs;
      return { pages, dbs, blockSel: [] };
    }, () => this.persist());
  }
  undo() { this.syncTail(); const h = this.histFor(); if (h.idx > 0) { this.applyHist(h.idx - 1); this.toast('Undo'); } }
  redo() { const h = this.histFor(); if (h.idx < h.stack.length - 1) { this.applyHist(h.idx + 1); this.toast('Redo'); } }
  ...
  flashBlock(id) {
    setTimeout(() => {
      const el = this._els[id] || document.querySelector('[data-bid="' + id + '"]');
