...
(function () {
  ...
  /* Backspace, told where the invisible characters are.
     ...
     block-level Backspace belongs there), or null when the character in front
     of the caret is an ordinary one and the browser is already right. */
  var CLOSERS = ['```', '**', '__', '~~', '==', '`', '*', '_'];
  ...
  function backspaceAt(text, at) {
    ...
    /* an ordinary character with nothing to tidy up after it — leave the key
       to the browser, which does it without a re-render */
    if (!hidden && !cut) return null;
  ...
  window.AMD = {
    ...
    markMap: markMap, backspaceAt: backspaceAt,
