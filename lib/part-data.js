...
AlamzaParts.register(class {
  ...
  /* The database's name in words, for the sidebar badge and the Data & sync
     screen. One method rather than a conditional at each call site, so a
     backend added later is named in exactly one place. */
  backendLabel() {
    const names = {
      rtdb: 'Realtime Database',
      firestore: 'Firestore',
      rest: 'Your API',
      routing: 'Split storage',
      local: 'This browser'
    };
    return names[AStore.backendName] || (AStore.backendName || 'Storage');
  }
  ...
  /* A page is either this account's own, or one shared WITH it. There is no
     third case: the role used to be read from a list of invitations kept in
     this workspace, which no other account can see, so it only ever described
     invitations this account had sent to itself. `shared` is set by the one
     path that can put someone else's page in memory — openShared() — and the
     rules behind it grant reading and nothing more. */
  myRole(page) {
    const p = page || this.state.pages[this.state.pageId];
    if (!p) return 'owner';
    return p.shared ? 'viewer' : 'owner';
  }
  canEdit(page) { const r = this.myRole(page); return r === 'owner' || r === 'editor'; }
  canComment(page) { const r = this.myRole(page); return r !== 'viewer'; }

  /* May this reader write this page's HISTORY — take a snapshot, restore one,
     delete one?
     Not `isReadOnly()`, which is deliberately true while previewing an old
     version: "Restore…" is offered from inside that preview and has to keep
     working. What matters here is the role the invite carries and whether the
     page is in the Trash. None of the three asked at all, so a viewer could
     overwrite a page they only have view access to, and all three were offered
     on a trashed page directly beneath a banner saying editing was disabled. */
  canWriteHistory(page) {
    const p = page || this.state.pages[this.state.pageId];
    if (!p || p.trashed) return false;
    if (this.state.route === 'public') return false;
  ...
  patchPage(id, patch, quiet) {
    this.setState(s => {
      ...
      this.persist();
      /* a page with readers keeps its shared copy current — a mirror that
         lags the page is a guest reading yesterday's note without knowing */
      if (patch && patch.blocks && this.scheduleShareSync) this.scheduleShareSync(id);
    });
  }
