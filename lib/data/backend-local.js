  ...
  function factory(config) {
    ...
    return D.defineBackend({
      ...
      watch: function (scope, col, handlers) {
        wire();
        var w = { scopeKey: scopeKey(scope), col: col, handlers: handlers || {}, live: true };
        watchers.push(w);
        /* Replay what is already stored, exactly like onChildAdded — the
           sidebar is painted from it, so a backend that only reports future
           changes boots the app empty.

           ASYNCHRONOUSLY, though. localStorage can answer instantly and this
           used to run inside watch(), which no networked backend can do. That
           made the demo the one place where a caller could see data before
           watch() had returned, and anything built on that timing would work
           here and fail on every real database. Being the reference
           implementation means matching the slow ones, not being faster. */
        Promise.resolve().then(function () {
          if (!w.live) return;                       /* unsubscribed before it landed */
          var have = childrenOf(scope, col);
          Object.keys(have).sort().forEach(function (k) {
            if (w.live && w.handlers.added) { try { w.handlers.added(k, have[k]); } catch (e) {} }
          });
        });
        return function () {
          w.live = false;
