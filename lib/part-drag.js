...
AlamzaParts.register(class {
  ...
  rowGrab(dbId, rowId, e, viewId) {
    ...
    const up = (ev) => {
      ...
      if (z && k >= 0 && !(z.srcAt >= 0 && z.srcAt === k)) {
        ...
        this._noFlip = true;
        const res = this.applyRowDrop(dbId, viewId, rowId, z.group, target);
        this._noFlip = false;
        changed = res.moved || res.regrouped;
      } else if (z && z.group != null) {
        this._noFlip = true;
        changed = this.setRowGroup(dbId, viewId, rowId, z.group);
        this._noFlip = false;
      }
  ...
  patchDb(dbId, fn, noHist) {
    ...
    if (!noHist) this.syncTail();
    const run = () => this.setState(s => {
      const dbs = { ...s.dbs }; const d = JSON.parse(JSON.stringify(dbs[dbId])); fn(d); dbs[dbId] = d; return { dbs };
    }, () => { this.persist(); if (!noHist) this.syncTail(); });
    if (this._noFlip) run(); else this.flipRows(run);
  }
});
