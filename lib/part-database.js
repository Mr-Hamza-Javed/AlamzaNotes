...
AlamzaParts.register(class {
  ...
  cellText(pr, r) {
    ...
    /* A date cell stores `YYYY-MM-DD`, which is what the editor speaks and
       what sorts correctly — it is not what anyone wants to READ. Text that
       is not a date at all (a column retyped from text, an import) is shown
       as it stands rather than blanked, so nothing disappears silently. */
    if (pr.type === 'date') return this.fmtDate(v) || (this.isBlank(v) ? '—' : String(v));
  ...
  /* `<input type="date">` speaks `YYYY-MM-DD` and nothing else: hand it
     anything it cannot parse and it renders EMPTY, which used to be committed
     straight back over the real value the moment the field lost focus. Both
     halves of that are fixed — this normalises what the editor is given, and
     `cellEdit.dirty` stops an untouched field writing at all. */
  dateInput(v) {
    const n = this.dateNum(v);
    if (n == null) return '';
    const d = new Date(n), p = (x) => (x < 10 ? '0' : '') + x;
    return d.getUTCFullYear() + '-' + p(d.getUTCMonth() + 1) + '-' + p(d.getUTCDate());
  }
  /* Read from the UTC fields and rebuilt as a local date, so a day never
     slips to the one before it west of Greenwich. */
  fmtDate(v) {
    const n = this.dateNum(v);
    if (n == null) return '';
    const d = new Date(n);
  ...
  boardGroups(db, view, rows, mk) {
    ...
    const known = (gp.options || []).map(o => this.optName(o)).filter(n => n != null && n !== '');
    const out = [];
    const placed = {};
  ...
  cmpCells(pr, ra, rb, desc) {
    ...
    if (pr.type === 'number') { const x = Number(a), y = Number(b); n = isNaN(x) || isNaN(y) ? 0 : x - y; }
      ...
      const names = (pr.options || []).map(o => this.optName(o));
  ...
  /* ============================ FIND ==================================
     A filter is part of the view — saved, shared, and lit in the toolbar. A
     find is not: it narrows what you are looking at right now and belongs to
     nobody else, so it lives in component state and is never written to the
     table. It searches the columns the view actually SHOWS, which is what the
     box sitting above them implies. */
  dbQ(dbId) { return String((this.state.dbQuery || {})[dbId] || '').trim(); }
  searchRows(db, view, rows) {
    const q = this.dbQ(db.id).toLowerCase();
    if (!q) return rows;
    const props = (db.props || []).filter(p => (view.hidden || []).indexOf(p.id) < 0);
    if (!props.length) return rows;
    return rows.filter(r => props.some(p => {
      const t = this.cellText(p, r);
      return t !== '—' && t.toLowerCase().indexOf(q) >= 0;
    }));
  }
  /* Filter, then find, then sort — in that order, once, so the table, Tab
     navigation and the board all agree on which rows exist and in what order.
     Two call sites used to build this by hand and could drift apart. */
  viewRows(db, view) {
    return this.sortRows(db, view, this.searchRows(db, view, this.applyFilters(db, view, (db.rows || []).slice())));
  }
  /* Set or clear the sort on ONE property, leaving the other keys in place —
     what a column header means by "sort by this". */
  sortByProp(dbId, viewId, propId, dir) {
    this.patchDb(dbId, d => {
      const v = (d.views || []).find(x => x.id === viewId); if (!v) return;
      const list = (v.sorts || []).slice();
      const i = list.findIndex(x => x.prop === propId);
      if (!dir) { if (i >= 0) list.splice(i, 1); }
      else if (i >= 0) list[i] = { prop: propId, dir: dir };
      else list.push({ prop: propId, dir: dir });
      v.sorts = list;
    });
  }
  /* ===================== A NEW ROW BELONGS TO ITS VIEW ==================
     Press New in a view filtered to "Status is Done" and the row was created,
     opened, and then — back in the table — simply not there. Notion pre-fills
     the conditions it can satisfy, so the row you just made is the row you
     can see. Only the conditions that NAME a value can be honoured; "is not",
     "is empty" and the ranges are left alone, and if the view still hides the
     row afterwards it says so rather than leaving the reader to wonder. */
  filterSeed(db, view) {
    const seed = {};
    const byProp = {};
    ((view && view.filters) || []).forEach(f => { (byProp[f.prop] = byProp[f.prop] || []).push(f); });
    Object.keys(byProp).forEach(pid => {
      const pr = (db.props || []).find(p => p.id === pid);
      if (!pr) return;
      /* the conditions on one property are OR'd, so satisfying the first
         that can be satisfied satisfies the group */
      const f = byProp[pid].find(x => {
        const op = x.op || 'is';
        if (pr.type === 'checkbox') return op === 'is';
        return (op === 'is' || op === 'contains') && !this.isBlank(x.value);
      });
      if (!f) return;
      seed[pid] = pr.type === 'checkbox' ? !!f.value : this.coerceCell(pr, f.value);
    });
    return seed;
  }
  addRowInView(dbId, viewId, extra) {
    const db = this.state.dbs[dbId];
    const view = db && (db.views || []).find(v => v.id === viewId);
    if (!db || !view) return this.addRow(dbId, extra || {}, true);
    const rid = this.addRow(dbId, Object.assign(this.filterSeed(db, view), extra || {}), true);
    if (!rid) return rid;                      // the rows are still arriving
    const d2 = this.state.dbs[dbId];
    const row = d2 && (d2.rows || []).find(r => r.id === rid);
    if (row && !this.applyFilters(d2, view, [row]).length) this.toast('Added — this view’s filters hide it');
    else if (row && !this.searchRows(d2, view, [row]).length) this.toast('Added — the find box hides it');
    return rid;
  }
  ...
  dbRowVals(db) {
    ...
    const blank = (loading, d) => ({
      ...
      /* every key the template reads, so a table that is still arriving is a
         skeleton rather than a handful of `undefined`s — an input handed
         `value: undefined` becomes uncontrolled and React complains */
      dbQuery: '', dbHasQuery: false, dbQueryBorder: 'var(--border)',
      dbSetQuery: () => {}, dbClearQuery: () => {},
      dbEmpty: false, dbHasRows: false, dbEmptyGlyph: '', dbEmptyTitle: '', dbEmptyBody: '',
      dbEmptyAction: '', dbEmptyRun: () => {},
      dbAddView: () => {}, dbAddProp: () => {},
      dbId: (d && d.id) || ''
    });
    ...
    const rows = this.viewRows(db, view);
    const total = db.rows.length;
    const q = this.dbQ(db.id);
    ...
    /* A column keeps the width the reader dragged it to. Stored on the VIEW,
       beside its filters and sorts, because two views of one table are two
       different tables to look at. */
    const widths = props.map((p, i) => {
      const w = (view.widths || {})[p.id];
      if (w) return Math.max(70, Math.min(760, w)) + 'px';
      return i === 0 ? '240px' : p.type === 'checkbox' ? '80px' : '140px';
    });
    ...
    const mk = (r) => ({
      ...
      cells: props.map((p, i) => {
        ...
        /* An unnamed row reads "Untitled" in its title column, the way it
           does everywhere else it appears — a bare em dash there says
           "no value" about the one column that is the row's name. */
        const txt = (i === 0 && this.isBlank(v)) ? 'Untitled' : this.cellText(p, r);
        ...
        return {
          ...
          text: txt,
          color: (txt === '—' || (i === 0 && this.isBlank(v))) ? 'var(--faint)' : 'var(--text)',
          ...
          editValue: editing && ed.draft !== undefined ? ed.draft : this.cellInputText(p, v),
          change: (e) => {
            const draft = e.target.value;
            /* `dirty` is what separates "the reader typed" from "the reader
               clicked here and clicked away" — see commit() */
            this.setState(st => (st.cellEdit ? { cellEdit: Object.assign({}, st.cellEdit, { draft, dirty: true }) } : null));
          },
          ...
          tap: (e) => {
            ...
            this.setState({ cellEdit: { dbId: db.id, rowId: r.id, propId: p.id, draft: this.cellInputText(p, v), dirty: false } });
          },
          /* Blur used to write whatever the field happened to hold, so merely
             CLICKING a cell and clicking away rewrote it — harmless for text,
             destructive for a date the browser's date input cannot represent:
             it rendered blank and the blank was saved over the real value.
             Nothing is written unless a keystroke actually landed. */
          commit: (e) => {
            const val = e.target.value;
            const mine = this.editingHere(db.id, r.id, p.id);
            if (mine && mine.dirty) this.setCell(db.id, r.id, p.id, val);
            if (mine) this.setState({ cellEdit: null });
          },
          key: (e) => {
            ...
            if (e.key === 'Tab') {
              e.preventDefault();
              const mine = this.editingHere(db.id, r.id, p.id);
              if (mine && mine.dirty) this.setCell(db.id, r.id, p.id, e.target.value);
    ...
    const groups = (view.type === 'board' ? this.boardGroups(db, view, rows, mk) : [])
      .map(g => Object.assign({}, g, { add: () => this.addRowInView(db.id, vid, groupSeed(g.key)) }));
    const nF = (view.filters || []).length, nS = (view.sorts || []).length;
    return {
      dbId: db.id,
      dbName: db.name,
      /* "3 items" while a filter or a find is hiding nine of them is a lie by
         omission — say how many there are as well as how many are showing */
      dbCount: rows.length === total
        ? total + (total === 1 ? ' item' : ' items')
        : rows.length + ' of ' + total,
      dbQuery: q, dbHasQuery: !!q,
      dbQueryBorder: q ? 'var(--accent)' : 'var(--border)',
      dbSetQuery: (e) => { const val = e.target.value; this.setState(st => ({ dbQuery: Object.assign({}, st.dbQuery, { [db.id]: val }) })); },
      dbClearQuery: () => this.setState(st => { const m = Object.assign({}, st.dbQuery); delete m[db.id]; return { dbQuery: m }; }),
      /* Three different situations rendered as the same blank space: a table
         nobody has added to, a find that matched nothing, and filters that
         hide every row. Each gets its own sentence and the button that ends
         it. A board is left out — its columns are the answer. */
      dbEmpty: !rows.length && view.type !== 'board',
      /* the foot of the table adds a row; when the panel below is showing it
         carries that action itself, and two of the same button is noise */
      dbHasRows: !!rows.length,
      dbEmptyGlyph: !total ? '🗂' : q ? '🔍' : '⊘',
      dbEmptyTitle: !total ? 'This table is empty' : q ? 'Nothing matches that' : 'Every row is filtered out',
      dbEmptyBody: !total
        ? 'Add the first row and it will appear here.'
        : q
          ? 'No visible column of these ' + total + ' rows contains “' + q + '”.'
          : 'All ' + total + ' rows fail this view’s filters.',
      dbEmptyAction: !total ? '＋ New row' : q ? 'Clear the find' : 'Clear the filters',
      dbEmptyRun: () => {
        if (!total) { this.addRowInView(db.id, vid); return; }
        if (q) { this.setState(st => { const m = Object.assign({}, st.dbQuery); delete m[db.id]; return { dbQuery: m }; }); return; }
        this.patchDb(db.id, d => { const v = (d.views || []).find(x => x.id === vid); if (v) v.filters = []; });
      },
      ...
      dbProps: props.map((p, i) => {
        const so = (view.sorts || []).find(x => x.prop === p.id);
        return {
          id: p.id, name: p.name, w: widths[i],
          glyph: (PROP_TYPES.find(t => t.id === p.type) || {}).glyph || 'Aa',
          /* which way this column is sorted, if it is — the toolbar's Sort
             button lit up but the column itself never said so */
          sortMark: so ? (so.dir === 'desc' ? '↓' : '↑') : '',
          resize: (e) => this.resizeCol(db.id, vid, p.id, e),
      ...
      dbAddRow: (e) => { if (e && e.stopPropagation) e.stopPropagation(); this.addRowInView(db.id, vid); }
    };
  }
  ...
  addRow(dbId, seed, open) {
    ...
    const rid = uid('r');
    this.patchDb(dbId, d => {
      /* Empty, not the literal word — the table and the row page both fall
         back to "Untitled" for display, so seeding the string only meant
         select-all-and-delete before you could type a name. */
      const cells = Object.assign({ [d.props[0].id]: '' }, seed || {});
  ...
  coerceCell(pr, val) {
    ...
    /* Stored as `YYYY-MM-DD` whatever it arrives as, so the editor, the
       comparator and the filter all read the same thing. Text that is not a
       date is KEPT — a column retyped from text still holds what it held, and
       `cellText` shows it. */
    if (pr.type === 'date') {
      if (val === '' || val == null) return '';
      return this.dateInput(val) || String(val);
    }
  ...
  /* What the field should START with — a date input only accepts YYYY-MM-DD.
     Named for the cell it fills: `editText` is already the block editor's, and
     every part is folded onto ONE prototype, so a second one would have
     silently replaced it. */
  cellInputText(pr, v) {
    if (pr && pr.type === 'date') return this.dateInput(v);
    return this.isBlank(v) ? '' : String(v);
  }
  /* the open editor, but only if it is still THIS cell. A Tab hand-off moves
     `cellEdit` to the next cell before the old field's blur arrives, and the
     old field must not then write into — or close — its successor. */
  editingHere(dbId, rowId, propId) {
    const ed = this.state.cellEdit;
    return (ed && ed.dbId === dbId && ed.rowId === rowId && ed.propId === propId) ? ed : null;
  }
  /* ---- dragging the seam between two column headers -------------------
     Painted straight onto the DOM while the pointer moves — a setState per
     pixel would re-render every row of the table — and written to the view
     once, on release. `noHist` because a column width is not an edit anyone
     wants ⌘Z to walk back through, and `_noFlip` because there is nothing to
     animate: the rows have not moved. */
  resizeCol(dbId, viewId, propId, e) {
    if (e.button !== 0) return;
    e.preventDefault(); e.stopPropagation();
    const head = e.currentTarget.closest && e.currentTarget.closest('[data-dbcol]');
    const w0 = head ? head.getBoundingClientRect().width : 140;
    const startX = e.clientX;
    const root = document.querySelector('[data-db="' + dbId + '"]');
    let w = null;
    const paint = (px) => {
      if (!root) return;
      [...root.querySelectorAll('[data-dbcol="' + propId + '"]')].forEach(n => { n.style.width = px + 'px'; });
    };
    const move = (ev) => {
      w = Math.max(70, Math.min(760, Math.round(w0 + ev.clientX - startX)));
      paint(w);
    };
    const up = () => {
      ...
      this.patchDb(dbId, d => {
        const v = (d.views || []).find(x => x.id === viewId); if (!v) return;
        v.widths = Object.assign({}, v.widths); v.widths[propId] = px;
      }, true);
      this._noFlip = false;
    };
  ...
  stepCell(dbId, viewId, rowId, propId, dir) {
    ...
    if (pi < 0) {
      /* Tabbed out of a cell that is not itself typable. The old arithmetic
         here read `findIndex(...) - dir` and could not survive a miss: a
         forward Tab past the last typable column produced -2, which then
         wrapped BACKWARDS into the previous row. Find the neighbour in the
         asked-for direction, and set `pi` so the `pi += dir` below lands on
         it — or past the end, so the wrap below is the one that fires. */
      const at = shown.findIndex(p => p.id === propId);
      if (at < 0) { this.setState({ cellEdit: null }); return; }
      let next = -1;
      if (dir > 0) next = props.findIndex(p => shown.indexOf(p) > at);
      else props.forEach((p, k) => { if (shown.indexOf(p) < at) next = k; });
      pi = next < 0 ? (dir > 0 ? props.length - 1 : 0) : next - dir;
    }
    pi += dir;
  ...
  /* An option is `{id, name, color}` — unless it is one of the bare strings
     an older workspace (and the demo seed) stores, in which case reading
     `.name` off it gives `undefined`. Every READER goes through here so it
     cannot matter; normAllDbs() below is what actually upgrades them. */
  optName(o) { return typeof o === 'string' ? o : (o && o.name); }
  /* Upgrade every table as it arrives. normDb() was written for exactly this
     and had no callers, so a table healed only when some menu happened to run
     normProp() over one of its properties — until then the board grouped by
     `undefined` (a column per option, all nameless and all empty, with the
     real values showing up as strays beside them) and every chip fell back to
     the default colour. */
  normAllDbs(dbs) {
    Object.keys(dbs || {}).forEach(id => { if (dbs[id]) this.normDb(dbs[id]); });
    return dbs || {};
  }
  normDb(d) {
  ...
  optOf(pr, name) {
    ...
    return (pr.options || []).find(o => this.optName(o) === name) || null;
  }
  optStyle(pr, name) {
  ...
  knownPeople(d) {
    ...
    /* people this workspace has shared a page with — the per-page member list
       is where that now lives; the old workspace-wide invite list never
       reached anyone and is no longer written to */
    Object.values(this.state.pages || {}).forEach(pg =>
      ((pg.share && pg.share.invites) || []).forEach(i => add(i.email)));
  ...
  deleteProp(dbId, propId) {
    /* The refusal below used to be silent: the menu item was offered, the
       click did nothing, and nothing said why. The title is the first
       property and always exists, so it is also the only way a table can be
       down to one — one message covers both. */
    const db0 = this.state.dbs[dbId];
    if (db0 && (db0.props || []).length && db0.props[0].id === propId) {
      this.toast('The title property cannot be removed'); return;
    }
    this.patchDb(dbId, d => {
