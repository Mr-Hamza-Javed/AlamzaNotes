...
AlamzaParts.register(class {
  ...
  renderVals() {
    ...
    /* sidebar tree — a page shared WITH this account belongs to its own
       section, not among this workspace's own notes */
    const all = Object.values(s.pages).filter(x => !x.trashed && !x.hidden && !x.shared);
    const tree = [];
    ...
    const indexKnown = !AStore.cloud || !!AStore.indexReady;
    ...
    const vlist = !wantList ? [] : [{
      ...
      bd: s.roVersion ? 'transparent' : 'var(--accent-tint)',
      dot: 'var(--accent)', line: versions.length ? 'var(--border)' : 'transparent',
      tagColor: 'var(--accent)',
      stat: false, deep: false, deepLabel: '', add: 0, del: 0, chg: 0,
      showFacts: false, showFactSep: false,
      ...
      const st = v.stat || (haveBoth
        ...
        : null);
      return {
        tag: 'v' + v.n, when: relTime(v.createdAt), message: v.message || 'No message',
        author: v.author || 'Alamza', auto: !!v.auto,
        actions: true, canRestore: canWriteHist, hasMenu: true,
        bg: v.id === s.roVersion ? 'var(--accent-soft)' : 'transparent',
        bd: v.id === s.roVersion ? 'var(--accent-tint)' : 'transparent',
        dot: v.id === s.roVersion ? 'var(--accent)' : 'var(--faint)',
        /* the timeline connects DOWNWARD, so the oldest snapshot ends it */
        line: i === versions.length - 1 ? 'transparent' : 'var(--border)',
        tagColor: 'var(--muted)',
        /* "+0 −0 ~0" is three numbers that say nothing; a snapshot whose root
           did not move is described by its nested count instead */
        stat: !!(st && (st.added || st.removed || st.changed)),
        add: st ? st.added : 0, del: st ? st.removed : 0, chg: st ? st.changed : 0,
        ...
        /* The facts line holds the counts and the reach. It carries its own
           separator rather than the template guessing, because either half can
           be absent and a stranded "·" is the sort of thing nobody notices
           until it ships. */
        showFacts: !!(st && (st.added || st.removed || st.changed)) || !!(v.deepStat && v.deepStat.pages > 1),
        showFactSep: !!(st && (st.added || st.removed || st.changed)) && !!(v.deepStat && v.deepStat.pages > 1),
    ...
    return {
      ...
      /* Name the database that is ACTUALLY running. It used to say "Firebase"
         because there was only one thing it could be; now lib/config.js can
         point the app anywhere, and the one line of feedback that a change of
         config took effect is this badge. */
      storeLabel: AStore.cloud
        ? (this.backendLabel() + ' · synced')
        : AStore.demo ? 'Demo · this browser' : 'Local storage',
      ...
      bodyLoading: !!(p && !p.blocks && AStore.cloud)
      ...
      /* ---- pages other people have shared with this account ---- */
      hasShared: (s.shares || []).length > 0,
      sharedList: (s.shares || []).map(x => {
        const pg = s.pages[x.pageId];
        return {
          id: x.pageId,
          title: (pg && pg.title) || x.title || 'Untitled',
          iconEl: this.iconEl(pg || { icon: x.icon }, 16),
          owner: (pg && pg.ownerName) || x.ownerName || 'Someone',
          loading: !pg,
          bg: s.pageId === x.pageId ? 'var(--row)' : 'transparent',
          color: s.pageId === x.pageId ? 'var(--text)' : 'var(--muted)',
          weight: s.pageId === x.pageId ? '600' : '500',
          open: () => { if (s.pages[x.pageId]) this.openPage(x.pageId); else this.openShared(x.pageId, true); },
          leave: (e) => { e.stopPropagation(); this.leaveShared(x.pageId); }
        };
      }),
      /* the banner on a page that is not this account's to edit */
      sharedBanner: !!(p && p.shared),
      sharedBannerText: p && p.shared
        ? ((p.ownerName || 'Someone') + ' shared this page with you. It updates when they edit it, and you cannot change it.')
        : '',
      leaveThisShare: () => { if (p && p.shared) this.leaveShared(p.id); },
