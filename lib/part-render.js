...
AlamzaParts.register(class {
  ...
  renderVals() {
    ...
    const vlist = !wantList ? [] : [{
      ...
      bg: s.diffB === 'current' && s.panel === 'versions' ? 'var(--accent-soft)' : 'transparent',
      bd: 'transparent', dot: 'var(--accent)', line: 'var(--border)', tagColor: 'var(--accent)',
      stat: false, canRestore: false, select: () => {}, context: () => {},
      ...
      return {
        ...
        actions: true, canRestore: true, bg: 'transparent', bd: 'transparent', dot: 'var(--faint)', line: 'var(--border)',
    ...
    return {
      ...
      readOnlyLabel: s.roVersion ? 'Read-only preview of ' + (this.versionById(s.roVersion) ? 'v' + this.versionById(s.roVersion).n + ' — ' + this.versionById(s.roVersion).message : 'an old version') : '',
      ...
      noComments: !(this.activeBlocks() || []).some(b => b.comments && b.comments.length),
      closePanel2: () => this.setState({ panel: null }),
      versionList: versionsLoading ? [] : vlist,
      noVersions: !versionsLoading && versions.length === 0,
      versionsLoading,
      /* nothing may be authored against a history that has not arrived */
      newVersionOff: versionsLoading,
      newVersionBg: versionsLoading ? 'var(--soft)' : 'var(--accent)',
      newVersionColor: versionsLoading ? 'var(--faint)' : '#fff',
      newVersionLabel: versionsLoading ? 'Loading history…' : 'New version',
      scrollRef: (el) => {
