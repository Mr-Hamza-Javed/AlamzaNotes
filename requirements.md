...
- **Multi-block selection** — lasso from the gutter beside the text, `⌘A`
  ...
  blocks to copy or delete. Escape drops the selection. The lasso rectangle is anchored in **document** space and
...
- **Floating format bar** on any text selection: bold, italic, strike, code,
  ...
  `bold` and the toggle strips the pair, rather than swallowing the asterisks
  into `***bold***`. With no word under the caret the key does nothing:
